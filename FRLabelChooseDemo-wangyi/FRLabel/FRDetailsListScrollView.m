...
#import "FRListItemButton.h"

@interface FRDetailsListScrollView ()

@property (nonatomic,strong) UIView *sectionHeaderView;

@property (nonatomic,strong) NSMutableArray *allItems;

@property (nonatomic,strong) FRListItemButton *itemSelect;

@end

@implementation FRDetailsListScrollView
...
#pragma 更多频道标签
    ...
    for (int i =0; i <count1; i++) {
        FRListItemButton *item = [[FRListItemButton alloc] initWithFrame:CGRectMake(padding+(padding+kItemW)*(i% itemPerLine), padding+(kItemH + padding)*(i/itemPerLine), kItemW, kItemH)];
        item.longPressBlock = ^(){
    ...
    for (int i=0; i<count2; i++) {
        FRListItemButton *item = [[FRListItemButton alloc] initWithFrame:CGRectMake(padding+(padding+kItemW)*(i%itemPerLine),CGRectGetMaxY(self.sectionHeaderView.frame)+padding+(kItemH+padding)*(i/itemPerLine), kItemW, kItemH)];
        item.operationBlock = ^(animateType type, NSString *itemName, int index){
            if (self.opertionFromItemBlock) {
                self.opertionFromItemBlock(type,itemName,index);
            }
...
-(void)itemRespondFromListBarClickWithItemName:(NSString *)itemName{
    for (int i = 0 ; i<self.allItems.count; i++) {
        FRListItemButton *item = (FRListItemButton *)self.allItems[i];
